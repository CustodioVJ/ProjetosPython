...
def cria_banco():
    conn = sqlite3.connect('agenda_hash.db')
    cursor = conn.cursor()
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS Senha(Logon TEXT NOT NULL PRIMARY KEY""");
    cursor.execute("""INSERT INTO Senha(Logon) VALUES('123456')""");
    conn.close()
    UserChoose()
...
def Senha():
    a = input("Insira a senha: ")
    conn = sqlite3.connect('agenda.db')
    cursor = conn.cursor() 
    cursor.execute("""SELECT Logon FROM Senha""")
